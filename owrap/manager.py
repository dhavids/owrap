...
from .utils.paths import (
    RUN_LOG, EXEC_LOG, READ_LOG, INPUT_FILE,
    _read_config, get_plan_path,
)
from .utils.paths import context_path, context_lock_path
from .utils.paths import (
...
class Manager:
    ...
    def __init__(self, port=None):
        ...
        if port is not None and use_multi:
            ...
            placeholder = SERVERS_DIR / "pool_default.json"
            self._state_file = str(placeholder)  # unused placeholder
        else:
    ...
    def start(self, port=4096):
        ...
        while time.time() < deadline:
            ...
            if proc.poll() is not None:
                raise RuntimeError(
                    "opencode serve exited before producing a URL"
                )
        if url is None:
            proc.terminate()
            raise RuntimeError(
                "opencode serve did not produce a URL within 15s"
            )
    ...
    def stop(self):
        ...
        if pid is not None:
            try:
                ...
                if self._logger:
                    self._logger.info(
                        "stop: server pid=%s already gone", pid
                    )
        try:
        ...
        if url and not self._server_responsive(url):
            if self._logger:
                self._logger.warning(
                    "get_url: pid=%s alive but port not responding, "
                    "treating as down", pid,
                )
            return None
        return url
    ...
    def ensure_running(self, port=4096):
        ...
        if self._logger:
            self._logger.debug(
                "ensure_running: server not running, starting"
            )
    ...
    def log_time(self, log_time=True):
        ...
        if log_time:
            if preprocess > 0.05:
                print(
                    f"[timing] preprocess={preprocess:.1f}s  "
                    f"opencode={opencode:.1f}s  total={total:.1f}s"
                )
            else:
                print(
                    f"[timing] opencode={opencode:.1f}s  "
                    f"total={total:.1f}s"
                )
    ...
    def register_task(self, task_id, call_type: str = "task"):
        ...
        tasks[str(task_id)] = {
            "status": "active",
            "invocation_time": self._t_invocation,
            "call_type": call_type,
        }
        self._write_state(state)
    ...
    def complete_task(self, task_id):
        ...
        if self._t_cmd_start is not None and self._t_cmd_end is not None:
            ...
            entry["duration_s"] = round(
                self._t_cmd_end - self._t_cmd_start, 3
            )
            entry["total_s"] = round(
                self._t_cmd_end - entry["invocation_time"], 3
            )
    ...
    def cleanup_done_tasks(self):
        ...
        for task_id, entry in tasks.items():
            status = entry if isinstance(entry, str) else entry.get(
                "status", "active"
            )
            if status == "done" or server_died:
                to_remove.append(task_id)
        if self.session_id:
            tasks_dir = session_tasks_dir(self.session_id)
        else:
    ...
    def append_context_recent(
        ...
        try:
            ...
            try:
                ...
                new_entry = (
                    f"- {now} [{kind}] {title_short} (rc={rc}, {ctx_str})"
                )
            ...
            while True:
                ...
                try:
                    ...
                    if RUNNING_DIR.exists():
                        for sentinel in RUNNING_DIR.glob(
                            f"*_{self.session_id}*"
                        ):
                            has_active = True
                            break
                    ...
                    if not pool:
                        continue
                    best = min(
                        pool, key=lambda e: _active_load(e["url"])
                    )
                    least_busy_url = best["url"]
                    prompt = (
                        f"--executor Compress the following session "
                        f"context file. Keep all section headers. "
                    )
                    prompt += (
                        f"Condense entries to the most important facts. "
                        f"Preserve the structure. "
                    )
                    prompt += (
                        f"Return only the compressed content:\n\n{snapshot}"
                    )
                    cmd = (
                        f"opencode run --attach {least_busy_url} -- "
    ...
    def _kill_port_occupant(self, port):
        ...
        if pids:
            ...
            if self._logger:
                self._logger.info(
                    "kill_port_occupant: cleared pids=%s port=%d",
                    pids, port,
                )
