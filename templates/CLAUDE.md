...
| `--executor` / `--exec [path]` | Executor | Read plan → execute → mark completed steps `[x]` in plan file. Reads from path if given, else `owrap/docs/plan0.md`. |
| `--check` | Planner review | Read plan + `[x]` TODOs; chain all file reads: `~/bin/oread -f a && ~/bin/oread -f b && ...` — one combined output. Flag violations as `[ ]` TODOs. No code changes. |
...
| `--refresh` | Refresh | Run `~/bin/owrap refresh` in bash (re-prints orientation and restores session context). |

**Hire-first rule:** Delegate any non-thinking work via:
- `~/bin/oread -f <file>` — cat a file (instant for ≤8000 chars, else summarises)
- `~/bin/oread -f <file> -v` — full cat bypassing the 8000-char limit (instant)
...
- `~/bin/oread -f <file> -s [-p <style>]` — summarise via opencode; auto-detects style by extension; timeout scales with file size (45–180s); `-p` to override; `~/bin/oread --list-styles` for all styles
- `~/bin/oread -f <file> -d "..."` — targeted query via opencode (55s timeout; `-t <s>` to extend)
- **Multiple oreads:** always chain with `&&` in ONE foreground Bash call — never use `-i` or `run_in_background=True` for ~/bin/oread.
- `~/bin/orun --msg "..."` — short inline tasks (foreground, single line, ≤1024 chars)
- `~/bin/orun --msg "..." -t <s>` — same with custom timeout (default: 180s)
- `~/bin/orun` — file-based tasks (auto-background + ~/bin/owait)
- `~/bin/oexec` — multi-step plan execution (auto-background + ~/bin/owait)

**Direct `Read`, `cat`, `ls`, `grep` are allowed** — `~/bin/oread` recommended for large files and directories (auto-summarises, grepping).

**All planner tool use is blocked — no exceptions, no prompting.** The Edit, Write, and Bash tools will be denied by permissions. Read is permitted. Do not attempt them, not even inside compound commands (e.g. `ls && ~/bin/oread ...` is still a denied `ls`). Do not ask the user for permission to use a tool — it will always be denied and wastes a turn. The only direct operations the planner may perform are writing to `plan_<id>.md`, `self.md`, `CLAUDE.md`, and `## TODO` sections in project files. For everything else: `~/bin/oread` for reads, `~/bin/orun` for writes/edits/commands, `~/bin/oexec` for plan execution. If ~/bin/owrap is unavailable, fall back to `--taskf`/`--execf` — never direct tool use, never user prompting.

**`~/bin/owrap setup` path resolution:** if `project_root` or `research_folder` are not passed, ~/bin/owrap reads them from `configs/owrap.json`. If the stored value is a valid absolute directory it is used directly; if not, it is tried relative to `~`; if neither exists, `~` is used as fallback.

**Planner fallback rule:** If the planner itself cannot invoke `~/bin/oread`, `~/bin/orun`, or `~/bin/oexec` for any reason (permission rejection, shim unavailable, server down), fall back directly — no Python runner involved: for tasks, write to `owrap/docs/run/tasks/task0.md` then run `opencode run -- --taskf`; for plan execution, run `opencode run -- --execf <plan_path>`. Add `--dangerously-skip-permissions` if needed. Always foreground (no `run_in_background=True`, no ~/bin/owait).
...
- `~/bin/owrap finish <target>` — kill a running job by target (exec, task1, task2, msg1, …) — sends SIGTERM
...
- Parallel `~/bin/orun --msg`: `~/bin/orun -i <id> --msg "..."` with `run_in_background=True`; stdout prefixed `[m:<id>]`, log tagged. Pass `-i` first (shim normalises any position). Without `-i`: foreground as normal, no tagging.
- `~/bin/oread` — always foreground; no tagging, no background dispatch.
    - After dispatching with `run_in_background=True`, make no further tool calls — not `true` keepalives, not `~/bin/owrap stat`, nothing. The harness delivers the notification automatically when the task exits. Only if no notification arrives after the threshold (1min ~/bin/oread, 3min msg/task, 5min exec), investigate once with `~/bin/owrap stat <session_id>`. rc=0=ok, rc=2=timeout (rerun -t), rc=143=crashed. Parallel `--msg` limit: max 3 simultaneous — LLM contention causes timeouts above this. For 4–6 operations: write to the input file and call `~/bin/orun`; for 7+ steps: use a plan (`~/bin/oexec`).
- `~/bin/orun` (file task) and `~/bin/oexec` auto-background and call `~/bin/owait run`/`~/bin/owait exec` internally — use `run_in_background=True` in Bash tool and wait for task-notification
- Use `--fg` to force foreground (e.g. for parallel pre-staged `--id N` tasks)
- Parallel dispatch: write task → `~/bin/orun` (run_in_background=True) → `~/bin/owait input` (blocks until input clear, prints `input clear`) → write next task → repeat → `~/bin/owait run` per completion
...
The executor may edit: code files, `docs/changes/`, `memory/<research>.md`. May mark completed plan steps `[x]` in `plan_<session_id>.md` — **only** changing `[ ]` to `[x]` on a step line; no other edit to the plan file is permitted. Must NOT edit non-TODO content of `self.md`.

## Planner Sweeps

On every `--planner`, `--check`, or plan-creation run:
1. Remove `[x]`-marked steps from `### Steps` in `plan_<session_id>.md` (executor marks these when complete).
2. Move corresponding items to `## DONE` in the relevant project file if they were tracked there.
